...
use crate::config::{SPRITE_SIZE, SCALE, WINDOW_WIDTH, WINDOW_HEIGHT};
...
impl Enemy {
    ...
    pub fn update_with_speed_mult(&mut self, speed_mult: f32, player_x: f32) {
        if self.kind == EnemyKind::Boss {
            ...
            if self.pos.x + half > WINDOW_WIDTH as f32 {
                self.pos.x = WINDOW_WIDTH as f32 - half;
                self.speed = -self.speed.abs();
            }
        ...
        if self.kind == EnemyKind::MiniBoss {
            ...
            let player_center = vec2(player_x, WINDOW_HEIGHT as f32 * 0.5);
        ...
        if self.wobble_count >= 40 {
            ...
            self.target_x = self
                .target_x
                .clamp(10.0, WINDOW_WIDTH as f32 - 26.0);
        }

        self.pos.x += (self.target_x - self.pos.x) * 0.15;

        if self.pos.y > WINDOW_HEIGHT as f32 {
            if self.kind != EnemyKind::MiniBoss && self.kind != EnemyKind::Boss {
                self.pos.y = -100.0;
                self.pos.x = rand::gen_range(10.0, WINDOW_WIDTH as f32 - 26.0);
    ...
    //     // movimento vertical + leve seno (igual ao pygame)
    //     let time = get_time() as f32;
    //     self.pos.y += self.speed * dt * 60.0
    //         + (time * 10.0).sin() * 2.0;

    //     // contador de wobble (≈40 frames)
    //     self.wobble_count += 1;
    //     if self.wobble_count >= 40 {
    //         self.wobble_count = 0;
    ...
    //     // aproximação suave (igual ao original)
    //     self.pos.x += (self.target_x - self.pos.x) * 0.15;
